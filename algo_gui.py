...
def create_frame(parent, back_callback,way):
    ...
    def run():
        ...
        if way == 1:
            result, tag = prim_mst_logic(text, placeholder)
        else:
            result, tag = topo_sort_logic(text, placeholder)
        write_output(result, tag, output)
    
    run_btn = tk.Button(frame,text=BTN_TEXT,command=run,bg="#00aaff",fg="#012233",font=("Arial", 12, "bold"),activebackground="#002639")
